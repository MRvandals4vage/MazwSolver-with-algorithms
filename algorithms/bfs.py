...
if __name__ == "__main__":
    ...
    # parsing user input
    # example: python bfs.py --display=1 --maze_file=maze_1.csv
    ...
    if args.display == 1:
        ...
        # screen title
        pygame.display.set_caption(f"Solving using BFS: {address}")
        ...
        while not done:
            ...
            if run == True:
                sleep(0.01)
                child_gen, done = bfs.child_gen(grid)
            
                explored = [(node.x, node.y) for node in bfs.closedlist]
        ...
        print(f"Solving using BFS: {address}")
    ...
    csv_path = f"../mazes_output/bfs/bfs_{args.maze_file}"
